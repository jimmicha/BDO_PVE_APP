...
it('reviews and publishes a version, preserves pinned goals, migrates pending steps and rolls back',async()=>{
  const db=await openDatabase();try{await seedUsers(db);let s=await rpc(db,ALICE,'companion_snapshot');
    ...
    expect(s.steps.find((x:any)=>x.template_key==='necklace').category).toBe('accessory');
    expect(s.steps.find((x:any)=>x.template_key==='olvia_enrollment').category).toBeNull();
    ...
    expect(s.steps.find((x:any)=>x.template_key==='belt').category).toBe('accessory');
