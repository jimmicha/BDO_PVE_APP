...
5. **Configure real authentication.** Configure the tester signup hook, Google and Discord OAuth, verified SMTP sender (Resend, chosen 2026-09-10) and secure identity linking on the new project. Verify email signup, recovery, Google/Discord linking and system-browser Android return links with real test accounts. The frontend already offers Google, Discord and email/password sign-in plus in-app linking (added 2026-09-10); only the hosted provider/SMTP configuration remains, and that requires the owner's own Google Cloud, Discord Developer Portal and Resend accounts — no tool in this environment can configure Supabase Auth providers or SMTP. Present a concrete cost review before any purchase.
