...
Update 2026-09-10 (later): the Supabase connector is authorized and step 2 is complete. All ten local migrations are applied to taxhvwhachtrhtcdbnsz in order, the reviewed v1 catalog seed is installed (published), an admin beta_members allowlist entry is set, and delete-account is deployed with its import map and JWT verification enabled. Security/performance advisors show only expected informational findings. The read-only preflight confirms the companion_snapshot RPC is installed and reachable, anonymous access is denied, and email/Google settings match the original notes below. Step 3 (Google OAuth, SMTP, Before User Created hook) and step 4 (hosted two-account acceptance suite) remain outstanding; step 5 (switching .env.local and live deployment) has not been started — no traffic has been switched and no player data was touched. See NEXT_STEPS.md for the current sequence.
